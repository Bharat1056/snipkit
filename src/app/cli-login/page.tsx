...
export default async function CliLoginPage({ searchParams }: CliLoginPageProps) {
  ...
  if (!session?.user?.id) {
    return redirect("/sign-in?callbackUrl=" + encodeURIComponent("/cli-login" + (redirectUri ? `?redirect_uri=${encodeURIComponent(redirectUri)}` : "")));
  }
          ...
          <CardContent className="space-y-6">
            ...
            <div className="pt-4 border-t border-gray-200">
              <p className="text-xs text-center text-gray-500">
                CLI authentication is now complete. You can close this window and return to your terminal.
              </p>
            </div>
          </CardContent>
        </Card>
